...
void go_list(int n, int interval, time_range t1, time_range t2, time_range t3, time_range t4, int log_flag)
{
    ...
    while (req_out1 < n)
    {
        ...
        if(log_flag && (req_out1 % interval == 0) && req_out1 != req_show)
        {
            req_show = req_out1;
            printf("%4d:\n", req_out1);
            printf("\tFirst queue : current (In-Out): %5d %5d (%3d ), average: %3d\n",req_in1, req_out1, d1.curr_size, d1.sum_size/d1.out_request);
            printf("\tSecond queue: current (In-Out): %5d %5d (%3d ), average: %3d\n",req_in2, req_out2, d2.curr_size, d2.sum_size/d2.out_request);
        }
    }
    printf("%4d:\n", req_out1);
    printf("\tFirst queue : current (In-Out): %5d %5d (%3d ), average: %3d\n",req_in1, req_out1, d1.curr_size, d1.sum_size/d1.out_request);
    ...
    if (t_modelling > 0)
    {
        double out_err = fabs(100* fabs((time - t_modelling))/time);
        printf("Out error: %lf%%\n\n", out_err);
    }
